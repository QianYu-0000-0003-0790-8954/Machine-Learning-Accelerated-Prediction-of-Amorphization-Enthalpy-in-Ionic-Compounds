...
import torch
import torch.nn as nn  # Ensure this import exists
...
class Network(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        if self.reduce_output:
            if 'batch' in data:
                batch = data.batch.view(-1)  # Ensure batch is 1D
                #print(f"Batch shape: {batch.shape}")  # Should be [106]
            else:
                batch = torch.zeros(data.pos.size(0), dtype=torch.long, device=x.device)
                #print(f"Batch shape (no batch): {batch.shape}")  # Should be [106]

            x = scatter_mean(x, batch, dim=0)

            # Debugging: Print output shape after scatter_mean
            #print(f"Output shape after scatter_mean: {x.shape}")  # Should be [batch_size, hidden_dim]

        return x

class PeriodicNetwork(torch.nn.Module):
    def __init__(
        self,
        in_dim,
        em_dim,
        ...
        self.em = nn.Linear(in_dim, em_dim)          # Linear layer from in_dim to em_dim
        self.em_z = nn.Linear(em_dim, em_dim)        # **New** Linear layer for z
        ...
        self.fea_update = nn.Sequential(
            ...
            nn.Linear(out_dim + feature_dim, out_dim + feature_dim),
            nn.SiLU(),
            nn.Linear(out_dim + feature_dim, num_properties)  # Final layer outputs num_properties
        )

    def forward(self, data):
        data.x = F.relu(self.em(data.x))           # [batch_size, 64]
        data.z = F.relu(self.em_z(data.x))         # [batch_size, 64]
        output = self.network(data)                # [batch_size, hidden_dim]
        
        # Debugging: Print output shape
        #print(f"Output shape from network: {output.shape}")  # Should be [batch_size, hidden_dim]
        
        feature = data.feature
        feature = self.fea_em(self.batch_norm(feature))
        output = torch.cat((output, feature), dim=1)
        output = self.fea_update(output)             # [batch_size, num_properties]
        return output
...
def train(
    ...
    scheduler=None,
    device="cpu",
    patience=20  # <-- Add a patience parameter
):
    model.to(device)
    history = {
        'train_loss': [],
    ...
    for step in range(1, max_iter + 1):
        ...
        for data in tqdm(dataloader_train, desc=f"Training Iteration {step}/{max_iter}"):
            ...
            # Reshape data.y to match output shape
        ...
        if avg_valid_loss < best_valid_loss:
            ...
            if no_improvement_steps >= patience:
                print(f"Validation loss has not improved for {patience} steps. Early stopping.")
                break
