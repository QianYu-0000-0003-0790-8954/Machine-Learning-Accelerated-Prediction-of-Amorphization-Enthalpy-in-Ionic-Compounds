...
def deload_data(filename, property_names):
    """
    Load and preprocess data for the given property.
    """
    df_ori = pd.read_csv(filename)
    descriptors = pd.read_csv('dataset/features.csv')
    #descriptors = pd.read_csv('dataset/final_merged_features_full_no_shear_modulus_edited.csv')
    ...
    feature_list = [
        ...
        'num__ElementProperty_MagpieData range SpaceGroupNumber'
    ]
    ...
    for index, row in tqdm(df_ori.iterrows(), total=df_ori.shape[0], desc="Loading Data"):
        ...
        if not values.isnull().any():
            ...
            try:
                struct = io.read(f'dataset/cifs/{material_id}.cif')
...
def train_valid_test_split(df, species, valid_size, test_size, seed=42, plot=False):
    """
    ...
    if insufficient_classes:
        print(f"Warning: {len(insufficient_classes)} classes have fewer than 2 samples. Adjusting stratification.")
    ...
    if test_size > 0:
        ...
        try:
            ...
            print(f"Stratification for validation/test split failed: {e}. Proceeding without stratification.")
            idx_valid, idx_test = train_test_split(
                idx_valid_test, 
                test_size=relative_test_size, 
                random_state=seed, 
